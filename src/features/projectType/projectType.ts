...
import { 
    ...
    resetData, 
    updateData 
} from "../../stores/features/projectTypeSlice";
...
export const createDatas = (datas:any) => {
    ...
    const {data:dataType, isError, isSuccess, isLoading, message:messageProjectType} = useSelector(
        (state : any) => state.projectType
    )

    useEffect(()=>{
        if(messageProjectType && isSuccess){
            if(!isLoading){
                set_message(messageProjectType.data);
                dispatch(resetData());
                navigate(-1);
            }
        }
    },[messageProjectType, isSuccess, isLoading])
