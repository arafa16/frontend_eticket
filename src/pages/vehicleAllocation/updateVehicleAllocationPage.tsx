...
import {
  getDataVehicleAllocationById,
  updateDataVehicleAllocation,
} from "../../features/vehicleAllocation/vehicleAllocation";

const updateVehicleAllocationPage = () => {
  const { id } = useParams();
  ...
  const { dataResult } = getDataVehicleAllocationById({ uuid: id });
  ...
  const { message, updateAction } = updateDataVehicleAllocation({
    uuid: id,
    name,
    sequence,
    is_select,
  ...
  return (
    <div>
      <div className="flex justify-end mt-8 gap-x-4">
        <Button size="xs" variant="primary" onClick={() => navigate(-1)}>
          Back
        </Button>
      </div>
      <div className="mt-4">
...
export default updateVehicleAllocationPage;
